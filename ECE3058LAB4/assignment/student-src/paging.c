...
void proc_init(pcb_t *proc) {
     ...
     */
     pfn_t base = free_frame();
     memset(mem + (base*PAGE_SIZE), 0, PAGE_SIZE);
     /** 
     for(int i = 0; i < PAGE_SIZE; i++) {
         mem[mem + (i*PAGE_SIZE)] = 0;
     }
     **/
     proc->saved_ptbr = base;
     frame_table[base].protected = 1;
}
...
uint8_t mem_access(vaddr_t address, char rw, uint8_t data) {
    ...
    pte_t* pte = (pte_t*) (mem + PTBR*PAGE_SIZE);
    pte += vpn;
    ...
    /*
        ...
        table entry.
    */
    uint8_t* pfn = (uint8_t*) (mem + pte->pfn * PAGE_SIZE + offset);
